...
public class Review {
    ...
    private Long id;    // 후기 고유 식별자

    @Column(name = "trip_id", nullable = false) // JPA: trip_id 컬럼으로 매핑, null 값 허용하지 않음
    private Long tripId; // 연관된 여행의 ID

    @Column(name = "user_id", nullable = false)
    private Long userId; // 후기 작성자(유저) ID

    @Column(columnDefinition = "TEXT", nullable = false) // JPA: TEXT 타입으로 매핑, null 값 허용하지 않음
    private String content; // 후기 내용

    @ElementCollection(fetch = FetchType.LAZY) // JPA: 컬렉션 타입 매핑, 지연 로딩 설정
    @CollectionTable(name = "review_files", joinColumns = @JoinColumn(name = "review_id")) // JPA: 파일 경로를 저장할 별도 테이블 설정
